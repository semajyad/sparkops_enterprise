-- Fix RLS policy for job_drafts table
...
-- Create correct RLS policy for job_drafts (using user_id column)
CREATE POLICY "Users can update their own job_drafts" ON public.job_drafts
    FOR UPDATE
    USING (user_id = auth.uid())
    WITH CHECK (user_id = auth.uid());

-- Add customer_email and customer_mobile columns to job_drafts table
